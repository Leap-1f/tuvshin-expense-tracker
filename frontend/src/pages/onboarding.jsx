...
export default function onBoard() {
  ...
  const balanceIsVisible = useIsVisible(balanceref);
  const doneref = useRef();
  const doneIsVisible = useIsVisible(doneref);
  function stepProgress() {
    ...
    if (step === "1") {
      ...
      progress[2].classList.remove("step-primary");
    } else if (step === "2") {
      progress[1].classList.add("step-primary");
      progress[2].classList.remove("step-primary");
    } else if (step === "3") {
      progress[1].classList.add("step-primary");
      progress[2].classList.add("step-primary");
    } else {
    }
  }
  function goTo() {
  ...
  function setSetting() {
    ...
    fetch("http://localhost:8080/user/setting", {
      method: "PATCH",
      mode: "cors",
      headers: {
      ...
      body: JSON.stringify({
        ...
        balance: document.getElementById("balancein").value,
      }),
    });
  }
  function checkWTF() {
  ...
  return (
    <div className="bg-white w-screen h-screen flex flex-col items-center">
      ...
      {/* below is progress */}
      <ul className="steps my-1 w-1/4 after:!h-1 ml-24 transition-all duration-300 mt-24">
        <li className={`step step-primary text-neutral gg`}>Currency</li>
        <li className={`step before:!h-1  text-neutral gg`}>Balance</li>
        <li className={`step before:!h-1  text-neutral gg`}>Finish</li>
      </ul>
      <div
        className="overflow-scroll flex flex-row  item-center scroll w-screen carousel carousel-center"
        id="container"
        onScroll={() => {
          checkWTF();
          stepProgress();
      ...
      >
        {/* below is currency */}
        <div
          className="w-screen flex justify-center align-center  flex-shrink-0 carousel-item relative"
          id="currency"
        >
          <div
            className="flex flex-col gap-4 justify-center items-center mt-32 w-1/6 "
            ref={currencyref}
            id="currency"
          >
            ...
            <select
              className="select select-bordered bg-base-100 border-base-300 border-2 rounded-lg w-full select-lg"
              id="select"
            >
        ...
        >
          ...
          >
            ...
            <a href="#done" className="w-full">
              <button
                className="btn w-full text-xl font-light  bg-primary text-white hover:bg-primary hover:text-black rounded-2xl"
                onClick={() => {
                  advanceStep("3");
                  stepProgress();
                }}
        ...
        >
          ...
          >
            ...
            <a href="/dashboard">
              <button
                className="btn text-xl font-light w-full bg-primary text-white hover:bg-primary hover:text-black rounded-2xl"
                onClick={() => {
                  advanceStep("1");
                  setSetting();
                }}
